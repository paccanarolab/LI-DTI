...
%% 
% * Output: Calculate and plot the Recall per target for the prospective evaluation 
...
test_fold = [YCoord XCoord ones(size(XCoord))];
%% *LI-DTI*
...
% load('../repository/prospective_evaluation/NeoDTI_prospective_predictions/results.mat');
...
%% 
% Plot the average recall per target for the prospective evaluation 
...
fprintf('\nProspective evaluation: recall per target\n');
fprintf('%-12s', 'Method');
...
for i = 1:length(methods)
    ...
    for ii = 1:length(topN)
        fprintf(' %6.4f', RecallTarget(i,ii));
    end
    fprintf('\n');
end
...
save('-mat7-binary', fullfile(outdir, 'prospective_recall_results_octave_target.mat'), ...
