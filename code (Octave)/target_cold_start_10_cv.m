...
% LI-DTI for the DTINet dataset saved in ../repository/cold_start_evaluation_1/target_cold_start  
...
for k = 1:10
    for j = 1:10
        load(['../data/luo_dataset/target_cold_start/our_method/cv_' int2str(k) '/test' int2str(j) '.mat']);
        tests{k}{j} = test_fold;
    end
end
%% 
% set the similarities thresholds (values from the paper)
% 
% in order: 
...
parfor j =1:10
    ...
    for i = 1:nfolds
        ...
        %initialise empty rows with the top k similar targets
        local_R = top_k_combined_similarity(local_R',prior_SIM_col,top_k);
        local_R = local_R';
...
% save('../repository/cold_start_evaluation_1/target_cold_start/our_method/our_method_predictions_10CV_2026.mat','our_method_predictions');